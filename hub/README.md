...
| Path | What it is |
|------|------------|
| [`standards/`](standards/) | The canonical, project-agnostic standards (the version other repos adopt): [git workflow](standards/git-workflow.md), [versioning](standards/versioning.md), [the notes system](standards/notes-system.md), [cross-project sync](standards/cross-project-sync.md), [AI context](standards/ai-context.md), [process reports](standards/process-reports.md) (the feedback loop — nodes report on each system run, the hub reviews to improve), plus the lifecycle runbooks [new-project setup](standards/new-project-setup.md) (greenfield), [onboarding an existing project](standards/onboarding-existing-project.md) (established repo joining), and [adopting updates](standards/adopting-updates.md) (pulling later changes); the [docs-site design system](standards/docs-site/) (the shared look + cross-linking that makes every project appear as a page of fairyfox.io); [deployment](standards/deployment.md) (static→Pages on the shared domain, built apps→Netlify, games as a recorded exception); [planning](standards/planning.md) (plan-before-execute by default); [supply-chain-hardening](standards/supply-chain-hardening.md) (least-privilege + SHA-pinned workflows, `SECURITY.md`, signed releases, mandatory branch protection); [dependencies](standards/dependencies.md) (upgrade aggressively behind a real test gate); [legal-docs](standards/legal-docs.md) (self-hosted, code-accurate Privacy/Terms/Cookies, plus the brand minimum); [coins](standards/coins.md) (the subtle, shared reading-engagement layer — earned by reading, never a gate, persistence striven for); [agent-tooling](standards/agent-tooling.md) (PowerShell + file tools, not the bash sandbox; execute-don't-hand-off; `.gitattributes` hygiene); [badges](standards/badges.md) (the README status-badge set); [testing](standards/testing.md) (pure/rendering split, real multi-layer tests, a regression per fix, preview visual changes); [engineering-quality](standards/engineering-quality.md) (no hacks, do the long work, docs + doc-comments, fearless refactoring behind the test gate); [repo-hygiene](standards/repo-hygiene.md) (doc-link + uncommitted-file gates, branch auto-delete with work-branch protection); [docs-lifecycle](standards/docs-lifecycle.md) (sweep current-state, never edit dated history); [research-capture](standards/research-capture.md) (understanding lands in the notes, same session); [working-rhythm](standards/working-rhythm.md) (task-tracking, background-then-foreground, adjacency-is-not-a-brief); [self-hosted-assets](standards/self-hosted-assets.md) (self-host fonts, no third-party hot-links); [farm-operating-model](standards/farm-operating-model.md) (the grow-daily integrated farm tier); [maintenance-sweep](standards/maintenance-sweep.md) (a documented whole-repo tidy back to a clean shipped baseline); and the [standards compliance audit](standards/compliance.md) (the enforcement pass — one on-request check that runs every standard's `Verify` section against the hub or a node). |
| [`templates/`](templates/) | Copy-paste starting files for a new or adopting project: a `CLAUDE.md`, a `VERSION`, a `.gitignore` snippet, a `notes/` skeleton, `project.gitattributes`, `SECURITY.md`, `dependabot.yml`, `branch-sync.yml`, `README-badges.md`, and a `legal/` page set. See [`templates/README.md`](templates/README.md). |
