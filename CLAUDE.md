...
together. Served at the `fairyfox.io` custom domain. (The repo was created as
`junebug12851.github.io`, a user site under the account's old name; after the account
was renamed to `1fairyfox`, the repo was renamed to `1fairyfox.github.io` on
2026-07-18 — restoring the user-site shape, which resolved a site-wide 404. The local
git remote may still use the redirecting old URL.)
...
| The site's visual design system changes (tokens, layout, components, cross-linking) — e.g. edits to `assets/css/main.css`, layouts, or includes | Keep the docs-site standard `hub/standards/docs-site/` in step, by default: the spec prose, `11-measurements-reference.md`, the bundled snapshot `docs-site/reference/main.css`, **and — when the chrome changes (`_includes/{head,header,subnav,footer}.html`, `assets/js/{reader,nav}.js`, or the chrome slice of `main.css`) — the vendored `docs-site/chrome/` bundle + its `chrome/VERSION`** (`12-shared-chrome.md`). fairyfox.io is the **master copy: manual review only, never auto-applied** here — but the *standard* (and the bundle) must mirror what the site actually is |
